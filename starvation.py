...
data=pd.read_csv('StarvationData2.csv')
...
n_inputs = 9	        #The 9 characterestics present for each thread				
n_neurons = 95		    #Number of neurons in each layer	
n_outputs = 2		    # 2 way classification
n_layers=3		        # Number of layers
learning_rate = 0.0001

x= tf.placeholder(tf.float32, [None, n_steps, n_inputs])
y = tf.placeholder(tf.int32, [None,n_outputs])
z=tf.Variable(tf.constant(1,dtype=tf.float32))
...
outputs=tf.unpack(tf.transpose(outputs,perm=[1,0,2]))			                            #this and the next line takes the last output of the output seq
in2full=outputs[-1]

logits = fully_connected(in2full, n_outputs, activation_fn=None)     	                    #getting the logits(shape=[batch_size,3]) based on input X
cross_entropy=tf.nn.softmax_cross_entropy_with_logits(logits,y)    			                #cross entropy error is takes as the error
